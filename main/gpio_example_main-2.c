		...
		case 5:  //BFRTC COMMAND
			...
			switch(uart_priv.uart_params) {
				...
				case 2:
					printf("BFRTC -F %ld!\n", uart_priv.data);
					xSemaphoreTake(xRTC_Semaphore, portMAX_DELAY);
    					myprivate.daycnt = (uint8_t)uart_priv.data;
    					xSemaphoreGive(xRTC_Semaphore);
    					printf("Updating daycnt in NVS Flash................\n");
    					int8_t daycnt = (int8_t)myprivate.daycnt;
      					err = nvs_set_i8(my_nvs_handle, "daycnt", daycnt);
      					printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
					break;
				case 3:
					printf("BFRTC -D %ld!\n", uart_priv.data);
					xSemaphoreTake(xRTC_Semaphore, portMAX_DELAY);
    					myprivate.daycnt_backup = (uint8_t)uart_priv.data;
    					xSemaphoreGive(xRTC_Semaphore);
    					printf("Updating daycnt in NVS Flash................\n");
    					int8_t daycnt1 = (int8_t)myprivate.daycnt_backup;
      					err = nvs_set_i8(my_nvs_handle, "daycntbck", daycnt1);
      					printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
				case 4:
					err = nvs_get_i8(my_nvs_handle, "daycnt", &temp);
					switch (err) 
					{
  						case ESP_OK:
    							printf("\n %d \n", (uint8_t)temp);
					...
					break;
				case 5:
					err = nvs_get_i8(my_nvs_handle, "daycntbck", &temp);
					switch (err) 
					{
  						case ESP_OK:
    							printf("\n %d \n", (uint8_t)temp);
    								myprivate.daycnt_backup = temp;
      							break;
    						case ESP_ERR_NVS_NOT_FOUND:
      							printf("BFRTC failed to read!\n");
      							break;
...
static void task_elevator_1_arrival(void* arg)
{
    ...
    uint8_t rtc_daycnt = 0;
    uint8_t rtc_daycnt1 = 0;
    for(;;) {
    		xSemaphoreTake(xDoorDly_Semaphore, portMAX_DELAY);
...
void app_main()
{
	...
	err = nvs_get_i8(my_nvs_handle, "daycnt", &divider);
  	switch (err) {
  		case ESP_OK:
    			printf("day count Read Done! day = %d\n", (uint8_t)divider);
      			myprivate.daycnt = divider;
      		break;
    		case ESP_ERR_NVS_NOT_FOUND:
      			printf("The day count is not initialized yet!\n");
      			printf("The day count is initializing now!\n");
      			divider = (int8_t)myprivate.daycnt;
      			err = nvs_set_i8(my_nvs_handle, "daycnt", divider);
      			printf((err != ESP_OK) ? "Failed!\n" : "Done\n"); 
      		break;
    		default :
     			 printf("Error (%s) reading!\n", esp_err_to_name(err));
	...
	err = nvs_get_i8(my_nvs_handle, "daycntbck", &divider);
  	switch (err) {
  		case ESP_OK:
    			printf("day count Read Done! day = %d\n", (uint8_t)divider);
      			myprivate.daycnt_backup = divider;
      		break;
    		case ESP_ERR_NVS_NOT_FOUND:
      			printf("The day count is not initialized yet!\n");
      			printf("The day count is initializing now!\n");
      			divider = (int8_t)myprivate.daycnt_backup;
      			err = nvs_set_i8(my_nvs_handle, "daycntbck", divider);
      			printf((err != ESP_OK) ? "Failed!\n" : "Done\n"); 
      		break;
    		default :
     			 printf("Error (%s) reading!\n", esp_err_to_name(err));
	...
	while(1) 
	{
          	...
          	if(day < myprivate.daycnt_backup)
		{
          		...
          		if(sec == 360 *24)
          		{
      				...
      				err = nvs_set_i8(my_nvs_handle, "daycnt", day);
      				printf((err != ESP_OK) ? "Failed!\n" : "Done\n"); 
      				//err = nvs_set_i8(my_nvs_handle, "daycntbck", day);
      				//printf((err != ESP_OK) ? "Failed!\n" : "Done\n");
